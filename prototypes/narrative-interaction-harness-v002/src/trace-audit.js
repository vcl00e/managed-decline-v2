...
export function auditRun(state, options = {}) {
  ...
  const rejectedConsumed = (state?.trace ?? []).filter(
    (event) => event?.type === 'action_rejected' && event.reason === 'already_consumed',
  );
  if (rejectedConsumed.length > 0) {
    warnings.push({
      code: 'REJECTED_DUPLICATE_INPUT',
      detail: `${rejectedConsumed.length} duplicate action input(s) were safely rejected`,
    });
  }
