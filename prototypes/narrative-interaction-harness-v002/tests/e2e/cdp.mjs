...
function keyMapping(key) {
  ...
  const special = {
    ...
    a: { key: 'a', code: 'KeyA', windowsVirtualKeyCode: 65, nativeVirtualKeyCode: 65, text: 'a' },
    s: { key: 's', code: 'KeyS', windowsVirtualKeyCode: 83, nativeVirtualKeyCode: 83, text: 's' },
    d: { key: 'd', code: 'KeyD', windowsVirtualKeyCode: 68, nativeVirtualKeyCode: 68, text: 'd' },
    '1': { key: '1', code: 'Digit1', windowsVirtualKeyCode: 49, nativeVirtualKeyCode: 49, text: '1' },
    '2': { key: '2', code: 'Digit2', windowsVirtualKeyCode: 50, nativeVirtualKeyCode: 50, text: '2' },
    '3': { key: '3', code: 'Digit3', windowsVirtualKeyCode: 51, nativeVirtualKeyCode: 51, text: '3' },
    '4': { key: '4', code: 'Digit4', windowsVirtualKeyCode: 52, nativeVirtualKeyCode: 52, text: '4' },
  };
  if (special[lower]) return special[lower];
  return { key, code: key, windowsVirtualKeyCode: key.charCodeAt(0), nativeVirtualKeyCode: key.charCodeAt(0) };
}
...
export async function injectFixture(client, rootInput, scenarioId) {
  const root = rootInput instanceof URL ? fileURLToPath(rootInput) : String(rootInput);
  const read = (relative) => fs.readFileSync(path.join(root, relative), 'utf8');

  const contractUrl = asDataModule(read('src/scenario-contract.js'));
  const traceUrl = asDataModule(read('src/trace-audit.js'));
  const renderUrl = asDataModule(read('src/render.js'));
  const vnUrl = asDataModule(read('src/vn.js'));
  const engineUrl = asDataModule(replaceImport(read('src/engine.js'), './scenario-contract.js', contractUrl));

  let createApp = read('src/create-app.js');
  ...
  const scenarioFile = scenarioId === 'parcel-fixture' ? 'scenarios/parcel.js' : 'scenarios/panel.js';
  let scenarioSource = read(scenarioFile);
  scenarioSource = replaceImport(scenarioSource, '../src/scenario-contract.js', contractUrl);
  const scenarioUrl = asDataModule(scenarioSource);
  const exportName = scenarioId === 'parcel-fixture' ? 'parcelScenario' : 'panelScenario';
  const bootstrapUrl = asDataModule(`
    import { mountScenarioApp } from '${createAppUrl}';
    import { ${exportName} as scenario } from '${scenarioUrl}';
...
export async function launchBrowser({ debugPort = 9432 } = {}) {
  const executable = findChromium();
  const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'md-harness-v002-chrome-'));
  ...
  async function navigate(url, { fixtureRoot = null, scenarioId = 'panel-fixture' } = {}) {
    const loaded = client.waitFor('Page.loadEventFired');
    await client.send('Page.navigate', { url });
    await loaded;
    const currentUrl = await client.evaluate('location.href');
    if (currentUrl.startsWith('chrome-error://') && fixtureRoot) {
      await injectFixture(client, fixtureRoot, scenarioId);
    }
