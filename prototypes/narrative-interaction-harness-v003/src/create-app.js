...
export function mountRecoveredApp({ scenario, root = document }) {
  ...
  function updatePersistentText() {
    situation.textContent = situationText();
    const last = runtime.state.visibleChanges.at(-1);
    feedback.textContent = last?.detail ?? scenario.startingFeedback ?? '';
  }
